...
function Dashboard() {
  ...
  return (
    <>
      <div className='fixed top-[26px] z-[99] bg-white w-[calc(100%-400px)]'>
        <MainHeader title={"Dashboard"} />
        <MainSearch />
      </div>
      <div
        id='dashboard-content'
        className='h-[96.3vh] poppins pt-[130px] pb-[20px]  w-full flex-col overflow-y-auto flex'
      >
        <div className='flex justify-end'>
          <div className='inline-flex border-[2px] text-[16px]  font-medium border-color-3 my-[20px] overflow-hidden rounded-[14px]'>
            <div
              onClick={() => isActive("food")}
              className={`main-search-query ${active && "active"}`}
            >
              Food
            </div>
            <div
              onClick={() => isActive("restaurant")}
              className={`main-search-query ${!active && "active"}`}
            >
              Restaurant
            </div>
